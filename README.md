...
### Day 31: Maximum Product of Three Numbers
* **Initial Thought:** Recognized that sorting the array would solve the problem in $O(n \log n)$ time, but optimized it to a single scan ($O(n)$) by tracking key elements. The maximum product always comes from either the three largest numbers or the two smallest (most negative) numbers multiplied by the largest number to capitalize on negative signs canceling out.
* **Performance:** Achieved a runtime of **2 ms (Beats 99.50%)** and **47.20 MB memory usage (Beats 90.06%)**. The solution maintains a time complexity of $O(n)$ by traversing the array exactly once. The space complexity is $O(1)$ since only a fixed number of integer variables are used to track the extrema, completely avoiding additional memory allocation.
* **Key Learning:** Reinforce how to eliminate sorting overhead by tracking only the necessary boundary elements (`max1`, `max2`, `max3`, `min1`, `min2`) dynamically. Mastered dealing with sign-flipping edge cases where negative values unexpectedly compound into the maximum absolute product when paired with a positive maximum.


### Day 32: Divide Array Into Arrays With Max Difference
* **Initial Thought:** Recognized that grouping elements with a minimal difference requires sorting the array first to bring similar values adjacent to one another. By iterating through the sorted array in chunks of three, a greedy approach can be applied. The maximum difference within any triplet always occurs between its last and first elements (`nums[i + 2] - nums[i]`). If this difference ever exceeds $k$, it is mathematically impossible to partition the remaining elements validly, so we return an empty array.
* **Performance:** Achieved a runtime of **27 ms (Beats 40.76%)** and **88.39 MB memory usage (Beats 68.01%)**. The solution runs with a time complexity of $O(n \log n)$ due to the initial sorting step, while the subsequent chunk validation takes linear $O(n)$ time. The space complexity is $O(n)$ to allocate the resulting 2D output array.
* **Key Learning:** Solidified the utility of sorting as a preprocessing step to convert a complex partitioning problem into a straightforward, contiguous chunk validation. Learned how to implement a fail-fast mechanism within a loop to immediately exit with an empty array the moment a single triplet constraint is violated.


### Day 33: Next Permutation
* **Initial Thought:** Realized that finding the next lexicographically greater permutation requires a right-to-left scan to identify the first inversion point where an element decreases (`nums[i] < nums[i + 1]`). Once this pivot is located, we scan from the right again to find the smallest number strictly greater than the pivot to swap with it. Reversing the entire suffix after the pivot then guarantees that the tail becomes the smallest possible lexicographical arrangement, effectively completing the transition in a single pass.
* **Performance:** Achieved a runtime of **0 ms (Beats 100.00%)** and **44.28 MB memory usage (Beats 96.76%)**. The algorithm executes with a time complexity of $O(n)$ since it passes through the array a maximum of three times, and a space complexity of $O(1)$ because all operations are performed strictly in place.
* **Key Learning:** Mastered the structural logic of permutations and how array reversals can be used to efficiently reset a descending subarray into its ascending baseline. Reinforced pointer manipulation strategies and learned how avoiding extra object allocations leads to flawless, top-tier performance metrics.


### Day 34: Merge Sorted Array
* **Initial Thought:** Recognized that standard front-to-back merging requires a temporary copy array to avoid overwriting elements in the main array. To optimize space, I implemented a right-to-left, back-to-front pointer strategy using indices `i`, `j`, and `k`. By comparing the largest available values at the ends of both arrays and placing them at the back of array `a`, we can safely populate the target array in-place without risking data loss.
* **Performance:** Achieved a runtime of **0 ms (Beats 100.00%)** and **43.40 MB memory usage (Beats 97.66%)**. The algorithm executes with a time complexity of $O(m + n)$ as it processes each element at most once, and a space complexity of $O(1)$ because it completely eliminates the need for extra allocations by utilizing the trailing empty slots.
* **Key Learning:** Solidified my understanding of in-place array manipulation and how altering the traversal direction (back-to-front instead of front-to-back) can eliminate spatial overhead. Learned to handle clean-up scenarios efficiently, ensuring that remaining elements in the second array are swept over seamlessly while elements already positioned in the primary array naturally remain in their correct sorted slots.


### Day 35: Reverse Integer
* **Initial Thought:** Extracted digits from the integer one by one from right to left using the modulo operator `% 10` and truncated the original number using division `/ 10`. To handle potential integer overflow during the reconstruction phase, I utilized a 64-bit `long` variable to safely accumulate the reversed digits before performing boundary checks against the 32-bit signed integer limits (`Integer.MAX_VALUE` and `Integer.MIN_VALUE`).
* **Performance:** Achieved a runtime of **1 ms (Beats 99.97%)** and **42.68 MB memory usage (Beats 45.43%)**. The algorithm runs with a time complexity of $O(\log_{10}(x))$ since the number of digits drops by a factor of 10 in each iteration, and a space complexity of $O(1)$ as it only requires a few primitive variables.
* **Key Learning:** Reinforced how to handle mathematical digit extraction without converting the number into a string. Gained practical experience in identifying and preventing integer overflow issues by using wider data types for intermediate calculations, ensuring that values exceeding the 32-bit threshold gracefully return zero as specified by the problem constraints.


### Day 36: Remove Duplicates from Sorted Array
* **Initial Thought:** Utilized a two-pointer approach to modify the array in-place, taking advantage of the fact that the input array is already sorted. By maintaining a slow pointer `k` to track the boundary of unique elements and a fast pointer `i` to scan through the array, I was able to overwrite duplicates sequentially without using any extra auxiliary collections.
* **Performance:** Achieved a runtime of **1 ms (Beats 77.44%)** and **46.55 MB memory usage (Beats 81.52%)**. The algorithm operates with a time complexity of $O(n)$ since it processes the array in a single linear pass, and a space complexity of $O(1)$ as the mutations are performed directly within the input array structure.
* **Key Learning:** Solidified the implementation mechanics of the two-pointer technique for in-place array manipulation. Enhanced familiarity with minimizing space complexity requirements by avoiding secondary data structures like Sets, and learned how to structure clean conditions to safely compare adjacent elements without running into index out of bounds exceptions.


### Day 37: Remove Element
* **Initial Thought:** Implemented a two-pointer technique to filter out target values directly in-place. By maintaining a placement pointer `k` to track where the next valid element belongs and an iterator `i` to scan the array, I sequentially shifted all non-target elements to the front of the array, completely bypassing the need for additional memory allocations.
* **Performance:** Achieved an optimized runtime of **0 ms (Beats 100.00%)** and **43.53 MB memory usage (Beats 42.91%)**. The solution runs with a linear time complexity of $O(n)$ as it scans the collection exactly once, and a constant space complexity of $O(1)$ due to processing the changes directly within the original array reference.
* **Key Learning:** Further mastered the application of tracking pointers for stream-filtering data inside fixed-size array structures. Gained insight into how conditional evaluation order can eliminate unnecessary element movements, reinforcing how to design clean array partitioning logic that fulfills custom verification criteria cleanly.


### Day 38: Integer to Roman
* **Initial Thought:** Implemented a greedy approach by mapping numeric boundaries to their respective Roman numeral representations using parallel arrays. By iteratively matching the remaining value against the largest possible decimal threshold, the solution appends the highest matching symbol and decrements the target number until the conversion fully settles.
* **Performance:** Achieved an optimized runtime of **3 ms (Beats 99.79%)** and **46.73 MB memory usage (Beats 8.61%)**. The solution runs with a constant time complexity of $O(1)$ since the input upper limit is strictly capped at 3999, and a constant space complexity of $O(1)$ due to utilizing fixed-size primitive mapping arrays and building the result string inline.
* **Key Learning:** Solidified the implementation of greedy value selection techniques for non-positional numeral translations. Gained insights into managing composite subtractive representations directly within global translation tables, highlighting how structured lookup arrays can completely eliminate complex conditional routing when splitting numeric sub-components.


### Day 39: Best Time to Buy and Sell Stock
* **Initial Thought:** Implemented a single-pass greedy approach to track the minimum price encountered while calculating the maximum possible profit on each subsequent day. By continually updating the lowest tracking boundary, the algorithm guarantees finding the optimal transaction margin by checking if selling at the current market price yields a better return than the previously recorded maximum.
* **Performance:** Achieved an optimized runtime of **1 ms (Beats 99.95%)** and **94.62 MB memory usage (Beats 15.66%)**. The solution runs with a linear time complexity of $O(n)$ by inspecting each day's price exactly once, and a constant space complexity of $O(1)$ since only a couple of primitive variables are used to maintain tracking bounds.
* **Key Learning:** Reinforces the efficiency of one-pass optimal tracking algorithms over brute-force nested evaluations. Developed a clear understanding of maintaining local extrema during array iteration to solve dynamic optimization challenges without incurring extra memory overhead.


### Day 40: Best Time to Buy and Sell Stock II
* **Initial Thought:** Implemented an optimized single-pass greedy approach to capture all positive price fluctuations. By comparing the current day's stock price directly with the previous day's price, the algorithm immediately locks in gains for every upward trend line, effectively simulating buying at local bottoms and selling at consecutive peaks without needing complex state tracking.
* **Performance:** Achieved a top-tier runtime performance of **0 ms (Beats 100.00%)** and **46.55 MB memory usage (Beats 31.37%)**. The logic maintains a strict linear time complexity of $O(n)$ by evaluating each transition point once, alongside an optimal constant space complexity of $O(1)$ through localized evaluation variables.
* **Key Learning:** Demonstrated the effectiveness of greedy strategies over traditional multi-state systems when arbitrary transactions are permitted. Learned how to breakdown aggregate interval accumulation down to localized day-to-day point mutations to guarantee absolute maximum optimization.


### Day 41: Container With Most Water
* **Initial Thought:** Implemented an optimized two-pointer approach to maximize the structural storage capacity between vertical lines. By positioning tracking boundaries at the absolute outer edges and systematically converging inwards, the algorithm calculates incremental volumetric limits based on the lower boundary height while dynamically squeezing the narrower line to aggressively seek higher potential peaks.
* **Performance:** Achieved a highly competitive runtime performance of **5 ms (Beats 82.94%)** and **77.30 MB memory usage (Beats 49.07%)**. The setup achieves a linear time complexity of $O(n)$ by inspecting each line boundary exactly once, coupled with an optimal constant space complexity of $O(1)$ by avoiding additional data arrays.
* **Key Learning:** Reinforced the power of boundary convergence mechanics over exhaustive combinatorial checking ($O(n^2)$). Mastered how eliminating strictly suboptimal area configurations based on local state constraints can optimize search space pruning.


### Day 42: Best Time to Buy and Sell Stock III
* **Initial Thought:** Implemented an optimized state-tracking dynamic programming approach to evaluate optimal entry and exit windows for completing at most two transactions. By maintaining four synchronous variables to record the net financial state after buying and selling at each step, the algorithm simulates reinvesting the first transaction's profits straight into the second buy requirement to maximize overall returns.
* **Performance:** Achieved a flawless runtime performance of **1 ms (Beats 100.00%)** and **75.76 MB memory usage (Beats 80.90%)**. The approach maintains an ideal linear time complexity of $O(n)$ by evaluating the array in a single pass, alongside an optimal constant space complexity of $O(1)$ by utilizing scalar variables instead of large multi-dimensional matrices.
* **Key Learning:** Mastered the technique of tracking dependent transactional states sequentially in a single pass. Learned how treating the second buy price as a net cost offset by prior profits eliminates redundant calculations and completely bypasses the need for costly DP arrays.


### Day 43: Third Maximum Number
* **Initial Thought:** Implemented an efficient single-pass tracking approach to find the third distinct maximum value without using full sorting routines. By keeping three distinct tracking references, the system dynamically cascades smaller bounds downwards whenever a higher unique maximum is found while safely ignoring duplicates to maintain strict element uniqueness.
* **Performance:** Achieved an accepted solution with a execution runtime of **4 ms (Beats 44.04%)** and a memory distribution of **46.47 MB (Beats 12.56%)**. The logic operates within an optimal time complexity of $O(n)$ by parsing the dataset exactly once, and preserves a strict auxiliary space complexity of $O(1)$ by using a fixed number of object references.
* **Key Learning:** Deepened understanding of object reference comparison in Java using the `Integer` class wrapper to comfortably address boundary constraints. Learned how conditional priority shifts can successfully extract top-K values while skipping duplicates without needing the overhead of arrays or auxiliary tracking collections.


### Day 44: Jump Game
* **Initial Thought:** Implemented an efficient single-pass greedy approach to determine if the last index can be reached. By maintaining a dynamic maximum reach frontier while iterating linearly, the algorithm determines if the current step falls within valid reachable boundaries, short-circuiting early if the destination index becomes attainable.
* **Performance:** Achieved an accepted solution with an execution runtime of **2 ms (Beats 89.21%)** and a memory distribution of **48.06 MB (Beats 23.48%)**. The logic operates within an optimal time complexity of $O(n)$ by evaluating each element at most once, and maintains a strict auxiliary space complexity of $O(1)$ by using a single local tracking variable.
* **Key Learning:** Reinforced the effectiveness of greedy strategies over exhaustive dynamic programming arrays when only global boundary reachability is required. Learned how tracking an expanding horizon of valid states eliminates the overhead of tracking individual path trajectories or multi-step branch decisions.


### Day 45: Missing Number
* **Initial Thought:** Implemented an optimal arithmetic progression approach using Gauss's summation formula to find the omitted value in a continuous numerical sequence. By calculating the theoretical total value of a complete range up to size $n$ and subtracting the combined sum of the array elements, the single missing value is extracted instantly without sorting.
* **Performance:** Achieved an accepted solution with an execution runtime of **0 ms (Beats 100.00%)** and a memory footprint of **47.45 MB (Beats 32.60%)**. The logic operates within an optimal linear time complexity of $O(n)$ by reading through the input items a single time, while keeping a strict auxiliary space complexity of $O(1)$ by using simple scalar variables.
* **Key Learning:** Reinforced how mathematical properties can bypass traditional search or tracking structures like HashSets, cutting memory overhead to zero. Realized that leveraging sequence totals completely eliminates structural sorting costs while guaranteeing a perfect execution speed score on the evaluation system.


### Day 46: Search Insert Position
* **Initial Thought:** Implemented an optimized logarithmic search approach using a binary search algorithm to pinpoint the exact index of a target element in a sorted, unique integer array. If the target element is absent, the search bounds cleanly collapse onto each other, allowing the algorithm to return the precise index where the element should be inserted sequentially.
* **Performance:** Achieved an accepted solution with an execution runtime of **0 ms (Beats 100.00%)** and a memory footprint of **44.55 MB (Beats 83.01%)**. The search space dynamically halves during each loop iteration, satisfying the strict time complexity constraint of $O(\log n)$ while preserving a minimal space complexity of $O(1)$ by avoiding recursion.
* **Key Learning:** Solidified the behavior of boundary tracking variables during an unsuccessful binary search, confirming that the `low` pointer naturally settles at the exact insertion index when a value is missing. This confirms how a standard search structure can be easily adapted to resolve tracking, insertion, and boundaries without needing secondary logic layers.


### Day 47: Maximum Average Subarray I
* **Initial Thought:** Implemented an optimized linear-scan approach using the sliding window technique to find the contiguous subarray of length `k` with the greatest average. By calculating the sum of the initial window first, the window dynamically shifts forward by adding the incoming element and subtracting the outgoing element, updating the maximum cumulative sum tracked along the way.
* **Performance:** Achieved an accepted solution with an execution runtime of **2 ms (Beats 99.83%)** and a memory footprint of **69.66 MB (Beats 44.19%)**. The algorithm processes the array elements in a single sequential pass, keeping the time complexity at a lean $O(n)$ while maintaining a constant space complexity of $O(1)$ by updating the values in place.
* **Key Learning:** Reinforced how the sliding window pattern eliminates redundant arithmetic operations by reusing the overlapping subarray sums instead of recalculating them from scratch. This converts a naive nested-loop solution into a highly efficient linear pipeline, demonstrating how to properly handle fractional values by casting the final division to a double precision float.


### Day 48: Gas Station
* **Initial Thought:** Implemented an efficient single-pass greedy approach to determine if a vehicle can complete a circular tour of gas stations. By tracking both the net global fuel balance and a localized tank balance, the algorithm determines if the total available gas covers the entire trip's cost. If the local tank drops below zero at any point, it logically eliminates all prior stations as viable starting candidates and resets the search from the next station forward.
* **Performance:** Achieved an accepted solution with an execution runtime of **3 ms (Beats 30.67%)** and a memory footprint of **119.68 MB (Beats 10.15%)**. The algorithm evaluates the two arrays concurrently in a single linear traversal, yielding a highly optimal time complexity of $O(n)$ while maintaining a strict space complexity of $O(1)$ by using a few constant pointer variables.
* **Key Learning:** Reinforced the utility of the greedy choice property in circular tracking problems, proving that if a car runs out of fuel between station A and station B, no station between A and B can successfully reach B either. It also highlighted how checking the aggregate sum at the very end can elegantly confirm the mathematical feasibility of a circular path without running redundant nested loops or physical array rotations.


### Day 49: Maximum Product Subarray
* **Initial Thought:** Implemented an optimized dynamic programming approach inspired by Kadane's algorithm to compute the maximum product of a contiguous subarray. Because multiplying by a negative number can instantly turn a small minimum value into a large maximum value, the algorithm continuously tracks both the maximum and minimum products ending at each position. When a negative integer is encountered, it seamlessly swaps the current maximum and minimum trackers before performing the multiplication.
* **Performance:** Achieved an accepted solution with an execution runtime of **2 ms (Beats 71.71%)** and a memory footprint of **47.25 MB (Beats 53.04%)**. The algorithm evaluates elements dynamically in a single linear pass, resulting in an optimal time complexity of \(O(n)\) and a strict space complexity of \(O(1)\) by utilizing simple constant variables instead of allocating auxiliary array storage.
* **Key Learning:** Reinforced the strategy of maintaining multiple running states (both minimum and maximum limits) to handle non-monotonic value fluctuations caused by sign flips. It also underscored how tracking a global maximum across every step effectively manages edge cases involving zeros and isolated elements without requiring redundant nested iterations or subarray slicing.


### Day 50: Candy
* **Initial Thought:** Implemented an optimized single-pass greedy approach leveraging peak-and-valley slope analysis to determine the absolute minimum candy distribution. By continuously tracking the lengths of rising sequences (`up`), falling sequences (`down`), and the highest point before a descent (`peak`), the algorithm directly accumulates candies step-by-step. If a descending sequence grows larger than the previous peak, the algorithm automatically increments the peak's candy count to satisfy the neighbor constraints seamlessly.
* **Performance:** Achieved an accepted solution with an exceptional execution runtime of **2 ms (Beats 99.87%)** and a memory footprint of **52.32 MB (Beats 28.33%)**. By eliminating the traditional two-pass array allocation strategy, this approach scans the input in a single linear pass, yielding an optimal time complexity of \(O(n)\) and a strict space complexity of \(O(1)\).
* **Key Learning:** Deepened understanding of replacing multi-pass array accumulation with dynamic slope and state tracking. It reinforced how modeling numeric array fluctuations as geometric peaks, valleys, and plateaus allows for local retro-active adjustments (like resolving trailing down-slopes against an earlier peak) without requiring auxiliary memory or redundant iterations.
